...
pub mod voting {
    ...
    pub fn initialize_poll(
        ...
        ctx.accounts.poll_account.poll_desc = description;
    ...
    pub fn vote(ctx: Context<Vote>, _poll_id: u64, _candidate: String) -> Result<()> {
        ...
        if current_time > (ctx.accounts.poll_account.poll_voting_end as i64) {
            return Err(CustomErrorCode::VotingEnded.into());
        }
        if current_time <= (ctx.accounts.poll_account.poll_voting_start as i64) {
            return Err(CustomErrorCode::VotingNotStarted.into());
        }
...
pub struct InitializePoll<'info> {
    ...
    #[account(init_if_needed,payer = singer,space = 8 + PollAccount::INIT_SPACE,seeds=[b"poll",poll_id.to_le_bytes().as_ref()],bump)]
...
pub struct InitializeCandidate<'info> {
    ...
    pub singer: Signer<'info>,
    pub poll_account: Account<'info, PollAccount>,
    #[account(init,payer = singer,space = 8 + Candidate::INIT_SPACE,seeds=[poll_id.to_le_bytes().as_ref(),candidate.as_ref()],bump)]
